...
function Player.getPoints(self)
	...
	balance = tonumber(result.getDataInt(resultId, "premium_points"))
	result.free(resultId)
	return balance
end

function Player.setPoints(self, amount)
	local accountId = self:getAccountId()
	db.query("UPDATE `accounts` SET `premium_points` = '" .. amount .. "' WHERE `id` = " .. accountId)
	return true
end
...
function Player.isInArea(fromPos, toPos)
    local position = player:getPosition()
    return position.x >= fromPos.x and position.y >= fromPos.y and position.x <= toPos.x and position.y <= toPos.y and position.z == fromPos.z
end
