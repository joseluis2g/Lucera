...
local exhaustPotions = Condition(CONDITION_EXHAUST_HEAL)
exhaustPotions:setTicks(1000)
...
function onUse(player, item, fromPosition, target, toPosition, isHotkey)
	...
	if player:getExhaustion(5493) > 0 then
		player:sendTextMessage(MESSAGE_STATUS_SMALL, 'You cannot use objects that fast.')
		return false
	end
	...
	--player:addCondition(exhaustPotions)
