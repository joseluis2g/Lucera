...
local function changeVocation(player, fromVocation, toVocation)
    local vocationsItems = {
        ...
        [3] = {
            ...
            [CONST_SLOT_AMMO] = {2544, 100, false, limitStorage = 4688, limit = 10}, -- 100 arrows
    ...
    if fromVocation == 0 then
        fromVocation = 4
        for skill = SKILL_CLUB, SKILL_AXE do
            player:setSkillLevel(skill, 15)
        end
        player:setSkillLevel(SKILL_DISTANCE, 13)
        player:addSkillTries(SKILL_DISTANCE, 0.63 * Vocation(4):getRequiredSkillTries(SKILL_DISTANCE, 14))
        player:setMagicLevel(2)
        player:addManaSpent(0.23 * Vocation(4):getRequiredManaSpent(3))
    end
 
    local vocation = Vocation(fromVocation)
    local totalManaSpent = player:getManaSpent()
    for i = 1, player:getBaseMagicLevel() do
        totalManaSpent = totalManaSpent + vocation:getRequiredManaSpent(i)
    end
    player:setMagicLevel(0)
    local tries = {}
    for skill = SKILL_FIST, SKILL_FISHING do
        local totalTries = player:getSkillTries(skill)
        for i = 11, player:getSkillLevel(skill) do
            totalTries = totalTries + vocation:getRequiredSkillTries(skill, i)
        end
        tries[skill] = totalTries
        player:setSkillLevel(skill, 10)
    end
    player:setVocation(toVocation)
    player:addManaSpent(totalManaSpent, false)
    for skill = SKILL_FIST, SKILL_FISHING do
        player:addSkillTries(skill, tries[skill], false)
    end
end
