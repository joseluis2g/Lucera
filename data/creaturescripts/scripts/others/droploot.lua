function onDeath(player, corpse, killer, mostDamage, unjustified, mostDamage_unjustified)
	...
	if amulet and amulet.itemid == ITEM_AMULETOFLOSS and not isInArray({SKULL_RED, SKULL_BLACK}, player:getSkull()) then
		if not isPlayer or not player:hasBlessing(BLESSING_TWIST_OF_FATE) then
		...
		for i = CONST_SLOT_HEAD, CONST_SLOT_AMMO do
			...
			if item then
				...
				if isPlayer and player:hasBlessing(BLESSING_ADVENTURER) then
					lossPercent = 0
				end
				if isInArray({SKULL_RED, SKULL_BLACK}, player:getSkull()) or math.random(item:isContainer() and 100 or 1000) <= lossPercent then
