...
local function creatureSayCallback(cid, type, msg)
	...
	local totalBlessPrice = 50000 -- getBlessingsCost(player:getLevel()) * 5 * 1.1
	...
	elseif msgcontains(msg, "outfit") then
		if player:getStorageValue(Storage.TheInquisition.Questline) == 16 then
			...
			player:getPosition():sendMagicEffect(CONST_ME_MAGIC_BLUE)
			npcHandler.topic[cid] = 0
		elseif player:getStorageValue(Storage.TheInquisition.Questline) > 20 and player:getStorageValue(Storage.TheInquisition.Questline) < 23 then
			npcHandler:say("Here is the first addon for your demon hunter outfit. Congratulations!", cid)
			player:addOutfitAddon(288, 1)
			player:addOutfitAddon(289, 1)
			player:getPosition():sendMagicEffect(CONST_ME_MAGIC_BLUE)
			npcHandler.topic[cid] = 0
