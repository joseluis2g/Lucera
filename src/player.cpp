...
void Player::updateItemCount() const
{
	if (client) {
		std::map<uint32_t, uint32_t> itemMap;
		getAllItemTypeCount(itemMap);

		std::map<uint16_t, uint16_t> items;
		for (const auto& it : itemMap) {
			const ItemType& item = Item::items.getItemType(it.first);
			items[item.clientId] = it.second;
		}

		client->sendItemCount(items);
	}
}
